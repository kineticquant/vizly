...
vizly v0.2.0 is a fully themable, low-boilerplate charting library over Apache ECharts. It targets teams that want ECharts without China-primary CDN defaults — English-first APIs, local or allowlisted JS, and native embeds in common Python web stacks.
...
6. **Locale and trust refinements** — `en-US` locale passed to `echarts.init`; trust scanner checks `<script src>` hosts only.
...
| Opt-in packs | `register_map_pack(name, geojson_path)` — e.g. China administrative GeoJSON you supply |
| Introspection | `list_bundled_maps()`, `list_opt_in_maps()` |
| Low-level | `load_map_geojson(name)`, `map_path(name)` |

China administrative map packs are **not** bundled by default.
