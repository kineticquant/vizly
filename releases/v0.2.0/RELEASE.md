...
vizly v0.2.0 is a fully themable, low-boilerplate charting library over Apache ECharts. Ship production charts in a few lines of Python (DataFrame in, HTML or JSON out) with high performance (local assets, one ECharts load per page) and native embeds for Streamlit, FastAPI, Flask, Django, HTMX, and Jupyter. Trusted defaults: no Chinese CDN forced in the backend (unlike many other Python ECharts wrappers). Worldwide maps by default.
